...
func JustificationInvalidPrepareData() *tests.MsgProcessingSpecTest {
	...
	prepareMsgs := []*qbft.SignedMessage{
		...
		testingutils.SignQBFTMsg(ks.Shares[3], types.OperatorID(3), &qbft.Message{
			...
			Round:      qbft.FirstRound,
			Identifier: []byte{1, 2, 3, 4},
			Data:       testingutils.PrepareDataBytes(nil),
		}),
	}
	...
	return &tests.MsgProcessingSpecTest{
		...
		ExpectedError:  "round change msg invalid: round change justification invalid: prepareData invalid: PrepareData data is invalid",
	}
}
